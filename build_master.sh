...
#./gradlew  pmd
#./gradlew jenkinsCheck
#./gradlew clean assemble -PbuildNumber=${BUILD_NUMBER} -PchangeLog="${CHANGE_LOG}"

./gradlew clean assembleRelease -PbuildNumber=${BUILD_NUMBER} -PchangeLog="${CHANGE_LOG}" lintProductionRelease crashlyticsUploadDistributionRelease
...
if [[ $f != *"unaligned"* ]]
		...
		if [[ $f == *"debug"* ]]
			...
			then
			output="../../../../build_results/500px_${BUILD_NUMBER}_${build_type}_${last_commit}.apk"
			cp "$f" "$output"
		fi
      fi
done
...
#mkdir -p /mnt/customshare/Products/500px/Android/#${BUILD_NUMBER}
#cp ./*.apk /mnt/customshare/Products/500px/Android/#${BUILD_NUMBER}
...
echo ' ' | tee -a build-results.txt
for f in *.apk; do
	echo "/mnt/customshare/Products/500px/Android/#${BUILD_NUMBER}/${f}"  | tee -a build-results.txt
	echo ' ' | tee -a build-results.txt
done
echo ' ' | tee -a build-results.txt
echo ' ' | tee -a build-results.txt
echo ' ' | tee -a build-results.txt
echo "Gitlab link: http://gitlab.kharkov.dbbest.com/android/500px/commit/${commit_hash}" | tee -a build-results.txt
echo ' ' | tee -a build-results.txt
echo ' '
echo '-----------------------------------'
echo ' '
